...
class ScriptArguments:
    ...
    target_key: Optional[str] = field(
        default="Truth_rewrite", metadata={"help": "the dataset name"}
    )
    chat: bool = field(
...
def main():
    ...
    # Load datasets
    ###############
    raw_datasets = get_qrecc(script_args, 
                             data_args.dataset_splits, 
                             target_key=script_args.target_key)
    ...
    raw_datasets = raw_datasets.map(apply_cqr_template, fn_kwargs={"tokenizer": tokenizer, "task": "sft"})
    train_dataset = raw_datasets["train"]
    eval_dataset = raw_datasets[script_args.eval_split]
    
    response_prefix = "[/INST]" if script_args.chat else "\n"
    collator = DataCollatorForCompletionOnlyLM(response_template= response_prefix + "<|rewrite|>",
                                               tokenizer=tokenizer)
