...
class StreamingDataset(IterableDataset):
    def __init__(self, elements, fn, records_per_element=1):
        super().__init__()
        self.elements = elements
        self.fn = fn
        self.records_per_element = records_per_element
        self.num_replicas = -1
        # Calculate the total length only if running in a distributed setting
        if dist.is_initialized():
            self.num_replicas = dist.get_world_size()
            self.total_length = self._calculate_length()
        else:
            self.total_length = len(elements) * records_per_element
    ...
    def _calculate_length(self):
        # Assuming each element generates a fixed number of records
        # Adjust this calculation if your actual dataset's length varies
        length = len(self.elements) * self.records_per_element
        if self.num_replicas != -1:
            length = length // self.num_replicas
        return length
